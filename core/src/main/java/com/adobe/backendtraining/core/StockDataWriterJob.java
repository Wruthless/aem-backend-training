package com.adobe.backendtraining.core;


import com.adobe.backendtraining.core.schedulers.StockImportScheduler;
...
import static com.day.cq.commons.JSONWriterUtil.put;


/**
 * Declare StockDataWriterJob as an OSGi component.
 */
@Component(
        // Parameters
        immediate = true,  // component should be activated upon deployment.
        service = JobConsumer.class,  // The component provides a service that implements the JobConsumer interface.
        // An array of property values for the component.
        // Specifies a property that corresponds to the job topics
        // the job consumer should handle.
        property = {
                // Property                       Value
                JobConsumer.PROPERTY_TOPICS + "=" + StockImportScheduler.JOB_TOPIC_STOCKIMPORT
        }
)
// A class implementing JobConsumer implements the process() method.
// This method is invoked when a job arrives for the job consumer.
public class StockDataWriterJob implements JobConsumer {
    ...
    /*
        ...
        is an instance of ResourceResolverFactory, automatically injected by OSGi
        because of the @Reference annotation. This factory can be used to obtain a
        ResourceResolver, which provides access to content resources.
     */
    @Reference
     ...
     *
     * @return JobConsumer.JobResult.[ OK | FAILED ]
     */
    @Override
    public JobResult process(Job job) {

        // Properties containing the stock symbol for the stock to be imported
        // and the URL to retrieve the stock data from.
        ...
        // Construct the URL to the JSON file with the stock data.
        String stockUrl = stock_url + symbol + ".json";

        // Sets up a connection to the structured stock Url
        ...
        try {
            ...
            // Convert data returned to a JSON object
            ...
            if (request != null) {
                ...
                try(JsonParser parser =
                            factory.createParser(new InputStreamReader((InputStream) request.getContent()))) {

                    // Create a map from the JsonParser
                    Map<String, String> allQuoteData =
                            objMapper.readValue(parser, new TypeReference<Map<String,String>>(){});

                    logger.info("Last trade for stock symbol {} was {}", symbol, allQuoteData.get("latestPrice"));

                    // Use map to write nodes and properties to JCR
    ...
    private JobResult writeToRepository(String stockSymbol, Map<String, String> quoteData) throws RepositoryException {
        ...
        // Get the service user (stock-import-service) that belongs to the training.core:training subservice
        Map<String, Object> serviceParams = new HashMap<>(  );

        // Get a ResourceResolver using a service user "backendtraining".
        serviceParams.put(ResourceResolverFactory.SUBSERVICE, "backendtraining");
        ...
        try (ResourceResolver resourceResolver = resourceResolverFactory.getServiceResourceResolver(serviceParams)) {
            ...
            LocalDateTime timePerLatestUpdate = LocalDateTime
                    .ofInstant(Instant.ofEpochMilli(latestUpdateTime), timeZone);
            ...
            // Stores timeOfUpdate as:  Hour:Minute AM/PM, TimeZone    e.g.   11:34 AM, EDT
            ...
            // Check to see if the trade node exists. If not, create it
            ...
            resourceResolver.commit(); // Write data into the JCR
